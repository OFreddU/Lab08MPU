// Librerias I2C
// la libreria MPU9250.h necesita I2Cdev.h, I2Cdev.h necesita Wire.h
#include "I2Cdev.h"
#include "MPU9250.h" // <--- LIBRERÍA CAMBIADA
#include "Wire.h"

MPU9250 sensor; // <--- OBJETO CAMBIADO
...
void setup() {
  ...
  Wire.begin();           //Iniciando I2C (usará pines 20 y 21 en el Mega)
...
void loop() {
  ...
  if (counter==100){
    ...
    if (p_az-16384>0) az_o--; // <-- Esto sigue siendo correcto
    else {az_o++;}
    ...
    //Calibrar el giroscopio a 0 deg/s en todos los ejes (ajustar el offset)
    if (p_gx>0) gx_o--;
    else {gx_o++;}
    if (p_gy>0) gy_o--;
    else {gy_o++;}
